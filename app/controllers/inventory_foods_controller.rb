class InventoryFoodsController < ApplicationController
  ...
  def create
    ...
    food_list.each do |food|
      next unless InventoryFood.where(foods_id: food.to_i, inventory_id: params[:id]).blank?

      new_inventory_food = InventoryFood.new(foods_id: food.to_i, quantity: params[:inventory_food][:quantity],
